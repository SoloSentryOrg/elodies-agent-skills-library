...
- Default to a professional `.docx`. Invoke the available Word/document skill and obey its create, render, inspect, accessibility, and privacy-QA requirements. Read [references/office-artifact-workflow.md](references/office-artifact-workflow.md) before creating DOCX, PPTX, XLSX, or PDF output. Use installed Microsoft Word for format-sensitive rendering and final visual inspection when it is available; do not substitute LibreOffice unless the user explicitly authorizes it.
